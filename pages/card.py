...
class CardPage:
    def __init__(self, app):
        self.app = app
        self.wait = WebDriverWait(self.app.wd, 70)

    def order_new_card_button(self):
        try:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.ORDER_NEW_CARD)
            )
    ...
    def new_virtual_card_type_button(self):
        return self.wait.until(
            EC.element_to_be_clickable(CardLocators.ORDER_NEW_VIRTUAL_CARD)
        )
    ...
    def sms_conformation(self, sms_code):
        ...
        try:
            self.wait.until(
                EC.frame_to_be_available_and_switch_to_it(CardLocators.IFRAME)
            )
            self.wait.until(
                EC.visibility_of_element_located(CardLocators.SMS_CODE_INPUT)
            ).clear()
            self.wait.until(
                EC.visibility_of_element_located(CardLocators.SMS_CODE_INPUT)
            ).send_keys(sms_code)
            self.wait.until(
                EC.visibility_of_element_located(CardLocators.CONFIRM_BUTTON)
            ).click()
    ...
    def success_title(self):
        try:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.SUCSESS_TITLE)
            )
        except NoSuchElementException:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.SUCSESS_TITLE)
    ...
    def type_card_dropdown(self):
        try:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.TYPE_SELECTOR)
            )
        except NoSuchElementException:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.TYPE_SELECTOR)
    ...
    def type_card_title_label(self, card_type: str):
        return self.app.wd.find_element_by_xpath(
            "//*[contains(@data-tag,'" + card_type + "')]" "//div[@class='span7']/h3"
        )
    ...
    def monthly_income_input(self):
        return self.wait.until(
            EC.visibility_of_element_located(CardLocators.MONTHLY_INCOME)
        )
    ...
    def credit_history_checkbox(self):
        return self.wait.until(
            EC.visibility_of_element_located(CardLocators.CREDIT_HISTORY)
        )

    def personal_data_checkbox(self):
        return self.wait.until(
            EC.visibility_of_element_located(CardLocators.PERSONAL_DATA)
        )

    def mobile_subscribe_checkbox(self):
        return self.wait.until(
            EC.visibility_of_element_located(CardLocators.MOBILE_SUBSCRIBE)
        )

    def inspect_credit_value_button(self):
        return self.wait.until(
            EC.visibility_of_element_located(CardLocators.INSPECT_BUTTON)
        )
    ...
    def card_success_alert(self):
        try:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.CARD_ALERT_SUCCESS)
            )
        except NoSuchElementException:
            return self.wait.until(
                EC.visibility_of_element_located(CardLocators.CARD_ALERT_SUCCESS)
